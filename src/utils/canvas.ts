...
export const downloadStrip = async (
  ...
  const paddingSide = frameStyle === 'blobby' ? 250 : 48;   // Very wide borders for blobby scale 3.0
  const paddingTop = frameStyle === 'blobby' ? 230 : 48;    // Very wide top border
  const gap = frameStyle === 'blobby' ? 150 : 24;           // Gap between photos
  const paddingBottom = frameStyle === 'blobby' ? 260 : 160; // Wide bottom border for text

  canvas.width = pxWidth + (paddingSide * 2);
  canvas.height = paddingTop + (pxHeight * numPhotos) + (gap * (numPhotos - 1)) + paddingBottom;

  // Fill background with color or gradient (all styles support color/gradient backgrounds!)
  ...
  for (let i = 0; i < Math.min(photos.length, numPhotos); i++) {
    const img = new Image();
    img.src = photos[i];
    ...
    // Draw photo with rounded corners matching the screen layout
    ctx.save();
    ctx.beginPath();
    const radius = 20; // Rounded corners for photo
    ctx.roundRect(paddingSide, y, pxWidth, pxHeight, radius);
    ctx.clip();
    ...
    if (frameStyle === 'blobby') {
      ...
      // Draw the Blobby Frame scaled 3x (matching scale-[3] on-screen)
      ...
      // Inner shadow/border for classic/minimal style
  ...
  if (frameStyle === 'classic') {
    ...
    drawFlower(ctx, 24, 984, 40, "#b3cde3", "#ffffff"); // Fallback check
  ...
  ctx.fillText('Dapinoy', canvas.width / 2, canvas.height - 75);
